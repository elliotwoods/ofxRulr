	...
	namespace Nodes {
		namespace Procedure {
			namespace Scan {
				...
				//----------
				void Graycode::exportDataSet(const string & filename) {
					if (this->suite) {
						this->suite->decoder.saveDataSet(filename);
				...
				void Graycode::populateInspector(InspectArguments & inspectArguments) {
					...
					inspector->add(new Widgets::Title("Scan", Widgets::Title::Level::H2));
					{
						...
						inspector->add(new Widgets::Button("Export ofxGraycode::DataSet...", [this]() {
							this->exportDataSet();
						}));
						inspector->add(new Widgets::Button("Import ofxGraycode::DataSet...", [this]() {
							try {
								this->importDataSet();
							}
							RULR_CATCH_ALL_TO_ALERT;
